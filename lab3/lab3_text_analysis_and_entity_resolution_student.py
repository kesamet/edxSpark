...
def countTokens(vendorRDD):
    ...
    """
    return sum(vendorRDD.map(lambda s: len(s[1])).collect())
...
def findBiggestRecord(vendorRDD):
    ...
    """
    biggestRec = (vendorRDD
                  .reduceByKey(lambda a, b: a+b)
                  .takeOrdered(1, lambda s: -1 * len(s[1])))
    return biggestRec
...
def idfs(corpus):
    ...
    """
    N = corpus.count()
    uniqueTokens = sc.parallelize(corpus.map(lambda s: list(set(s[1]))).reduce(lambda a, b: a + b))
...
similarities = (crossSmall
                .map(lambda s: computeSimilarity(s))
                .cache())
...
trueDupsTotal = trueDupsRDD.reduce(lambda x, y: x+y)
avgSimDups = float(trueDupsTotal)/trueDupsCount

nonDupsRDD = (sims
              .subtractByKey(goldStandard)
              .map(lambda s: s[1]))
nonDupsCount = nonDupsRDD.count()
nonDupsTotal = nonDupsRDD.reduce(lambda x, y: x+y)
avgSimNon = float(nonDupsTotal)/nonDupsCount
...
# TODO: Replace <FILL IN> with appropriate code
amazonNorms = amazonWeightsRDD.map(lambda s: (s[0], norm(s[1]))).collectAsMap()
amazonNormsBroadcast = sc.broadcast(amazonNorms)
googleNorms = googleWeightsRDD.map(lambda s: (s[0], norm(s[1]))).collectAsMap()
...
def fastCosineSimilarity(record):
    ...
    s = 0.0
    for token in tokens:
        if token in amazonWeightsBroadcast.value[amazonRec] and token in googleWeightsBroadcast.value[googleRec]:
...
similaritiesFullRDD = (commonTokens
                       .map(lambda s: fastCosineSimilarity(s))
                       .cache())
