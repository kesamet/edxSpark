...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    ave = float(sum(IDandRatingsTuple[1]))/count
    return (ID, (count, ave))
...
trainingAvgRating = float(trainingRDD.map(lambda s: s[2]).reduce(lambda a, b: a + b))/trainingRDD.count()
...
myRatedMovies = [
     (myUserID, 1088, 5.0), (myUserID, 1171, 5.0), (myUserID, 1195, 4.5), (myUserID, 1110, 4.5), (myUserID, 527, 4.0), 
     (myUserID, 1248, 5.0), (myUserID, 260, 5), (myUserID, 983, 3.5), (myUserID, 750, 4.0), (myUserID, 941, 4.0)
...
myUnratedMoviesRDD = (moviesRDD
                      .filter(lambda s: s[0] not in [ratedMovie[1] for ratedMovie in myRatedMovies])
                      .map(lambda s: (myUserID, s[0])))
